...
;(function(){
	$.nc_item = {
		...
		switch_segment_item:function(obj){
			...
			var target_seg = obj.parents(".segment_box").siblings(".cou_content").find(".seg_page"+obj.index());
			if(target_seg.length>0){
				console.log('0000');
				$(".cou_content .seg_page.show").fadeOut(function () {
					$(this).removeClass("show");
					target_seg.fadeIn(function () {
						$(this).addClass("show");
					})
				})
			}
...
.on("click",".item_view .item_option a.cancel",function(){	
	$.nc_item.modal_out({ target:"item_view" });
})
.on("click",".cart_item .item_text .icon_sm_edit",function(){
	$.nc_item.modal_show({ target:"item_view" });
})
